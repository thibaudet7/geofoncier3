...
// POST /api/spatial/transform - VERSION AMÉLIORÉE

router.post('/transform', async (req, res) => {
    try {
        const { coordinates, fromSRID, toSRID = 4326 } = req.body
        
        console.log('🔄 Demande de transformation:', { 
            fromSRID, 
            toSRID, 
        ...
        if (!Array.isArray(coordinates) || coordinates.length < 3) {
            return res.status(400).json({
                success: false,
                error: 'Au moins 3 coordonnées requises'
            })
        }

        // Si déjà en WGS84, retourner directement
        if (fromSRID === 4326 || fromSRID === 'wgs84') {
            return res.json({
                success: true,
                transformedCoordinates: coordinates,
                message: 'Déjà en WGS84, aucune transformation nécessaire'
            })
        }
        ...
        try {
            // Essayer d'abord la transformation via SpatialService (avec Python)
            result = await SpatialService.transformCoordinates(coordinates, fromSRID, toSRID)
            
            if (result.success) {
                console.log('✅ Transformation réussie via SpatialService');
                return res.json({
                    success: true,
                    transformedCoordinates: result.transformedCoordinates,
                    method: 'python'
                })
            } else {
                console.warn('⚠️ Échec SpatialService, essai conversion JavaScript...');
                throw new Error(result.error);
            }
            
        } catch (spatialError) {
            console.warn('⚠️ SpatialService échoué:', spatialError.message);
            
            // Fallback : conversion JavaScript simple
            try {
                const jsResult = performJavaScriptConversion(coordinates, fromSRID, toSRID);
                
                if (jsResult.success) {
                    console.log('✅ Transformation réussie via JavaScript');
                    return res.json({
                        success: true,
                        transformedCoordinates: jsResult.transformedCoordinates,
                        method: 'javascript',
                        warning: 'Conversion approximative utilisée'
                    })
                } else {
                ...
                return res.status(400).json({
                    success: false,
                    error: `Transformation impossible: ${spatialError.message}. Fallback JS: ${jsError.message}`
                })
            }
        }
        ...
        res.status(500).json({ 
            success: false,
            error: 'Erreur serveur lors de la transformation' 
        })
    }
})

// FONCTION DE CONVERSION JAVASCRIPT (FALLBACK)
function performJavaScriptConversion(coordinates, fromSRID, toSRID = 4326) {
    try {
        console.log('🟨 Conversion JavaScript pour', fromSRID, '→', toSRID);
        
        const transformedCoords = [];
        
        for (let i = 0; i < coordinates.length; i++) {
            const [coord1, coord2] = coordinates[i];
            let lat, lng;
            
            switch (fromSRID) {
                case 32632:
                case 'utm32':
                    // UTM32 vers WGS84 (approximation pour Cameroun occidental)
                    [lat, lng] = convertUTM32ToWGS84(coord1, coord2);
                    break;
                    
                case 32633:
                case 'utm33':
                    // UTM33 vers WGS84 (approximation pour Cameroun oriental)
                    [lat, lng] = convertUTM33ToWGS84(coord1, coord2);
                    break;
                    
                case 'douala':
                    // Douala 1948 vers WGS84 (approximation)
                    [lat, lng] = convertDoualaToWGS84(coord1, coord2);
                    break;
                    
                default:
                    // Tentative de guess intelligent
                    [lat, lng] = guessAndConvert(coord1, coord2);
                    break;
            }
            ...
            if (lat >= 1 && lat <= 13 && lng >= 8 && lng <= 17) {
                ...
                console.warn(`⚠️ Point ${i+1} hors Cameroun: ${lat}, ${lng}`);
                // Correction d'urgence
                const [correctedLat, correctedLng] = correctOutOfBounds(coord1, coord2);
                transformedCoords.push([correctedLat, correctedLng]);
            }
        }
...
function convertUTM32ToWGS84(northing, easting) {
    // Conversion approximative UTM32N → WGS84 pour le Cameroun
    const centralMeridian = 9.0;
    const falseEasting = 500000.0;
    
    const x = easting - falseEasting;
    const y = northing;
    
    // Facteurs approximatifs pour la région du Cameroun
    const lat = y / 111320.0;
    const lng = x / (111320.0 * Math.cos(lat * Math.PI / 180)) + centralMeridian;
    
    return [lat, lng];
}

function convertUTM33ToWGS84(northing, easting) {
    // Conversion approximative UTM33N → WGS84 pour le Cameroun
...
function convertDoualaToWGS84(y, x) {
    // Conversion approximative Douala 1948 → WGS84
    // Paramètres approximatifs pour la région de Douala
    const lat = y / 111320.0 + 4.0;
    const lng = x / 111320.0 + 9.7;
    
    return [lat, lng];
}

function guessAndConvert(coord1, coord2) {
    // Si ressemble déjà à WGS84 (Cameroun)
    if (coord1 >= 1 && coord1 <= 13 && coord2 >= 8 && coord2 <= 17) {
        return [coord1, coord2];
    }
    
    // Si ressemble à UTM (grandes valeurs)
    if (coord1 > 100000 && coord2 > 100000) {
        return convertUTM32ToWGS84(coord1, coord2);
    }
...
function correctOutOfBounds(coord1, coord2) {
    // Essayer d'inverser
    if (coord2 >= 1 && coord2 <= 13 && coord1 >= 8 && coord1 <= 17) {
        return [coord2, coord1];
    }
    
    // Essayer de diviser par un grand facteur
    let lat = coord1;
    let lng = coord2;
    
    if (lat > 100000) lat /= 100000;
    if (lng > 100000) lng /= 100000;
    
    if (lat >= 1 && lat <= 13 && lng >= 8 && lng <= 17) {
        return [lat, lng];
    }
    
    // En dernier recours, centre du Cameroun
    console.warn('⚠️ Utilisation coordonnées par défaut');
    return [6.0, 12.0];
}
